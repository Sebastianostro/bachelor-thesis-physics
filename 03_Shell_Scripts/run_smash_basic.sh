...
usage() {
    echo "Usage: $0 [-c config.yaml] [-d decay.txt] [-C container.sif] [-o output_root] [-r run_id] [-- smash_args]"
}
...
singularity exec $CONTAINER smash -i $CONFIG_FILE -d $DECAY_FILE -o $output_dir_smash/$RUN_ID/ -n $SMASH_EXTRA_ARGS
